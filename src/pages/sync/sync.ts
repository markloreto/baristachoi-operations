...
export class SyncPage {
  ...
  withConnect: any = {
    ...
    "products": { alias: "Products", col: "product_id", tables: ["inventories"], reference: [] },
  ...
  syncNow() {
    ...
    this.syncPull(["staffs", "roles", "products", "product_categories", "measurement_units", "attachments", "delivery_receipts", "inventories", "modules"], this.all)
  }
    ...
    if (inc < data.length) {
      this.myFunctionProvider.dbQuery("SELECT * FROM " + table + " WHERE id = ?", [data[inc].id]).then((rec: any) => {
        if (rec.length) {
          ...
          let data_omitted = _.omit(data[inc], ['created_at', 'updated_at', 'deleted_at'])
  ...
  syncPull(tables, all = false) {
    ...
    if (this.syncPullStep < tables.length) {
      ...
      this.syncPush(["products", "delivery_receipts", "inventories", "attachments"], false)
    }
  }
